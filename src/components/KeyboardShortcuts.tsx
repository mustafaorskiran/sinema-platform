...
const SHORTCUTS = [
  ...
  { key: 'g h', desc: 'Ana sayfaya git' },
  { key: 'g f', desc: 'Filmler sayfasına git' },
  { key: 'g d', desc: 'Diziler sayfasına git' },
  { key: 'g p', desc: 'Profilime git' },
  { key: 'g l', desc: 'İzleme listem' },
  { key: '?', desc: 'Bu yardım ekranını göster/gizle' },
  { key: 'Esc', desc: 'Kapat' },
]
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
      ...
      <div className="w-full max-w-sm rounded-2xl p-6" onClick={e => e.stopPropagation()}
        ...
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-base font-bold text-white">Klavye Kısayolları</h2>
        ...
        <p className="text-[10px] mt-4 text-center" style={{ color: 'rgba(255,255,255,0.2)' }}>
          Herhangi bir yerde <kbd className="px-1 py-0.5 rounded text-[9px]" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}>?</kbd> tuşuna bas
        </p>
      </div>
    </div>
  )
