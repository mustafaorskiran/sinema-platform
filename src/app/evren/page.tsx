...
export default async function EvrenPage() {
  const { t } = await getTranslations()
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        <h1 className="text-3xl font-black text-white mb-1">{t('evren.title')}</h1>
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: u.color }}>
                  {u.shortName} · {u.filmCount} {t('home.filmsLabel')}
                </p>
              </div>
            </div>
            <p className="text-xs mb-4 flex-1" style={{ color: 'rgba(255,255,255,0.4)' }}>{t(u.descKey)}</p>
            <div className="flex gap-2">
              {u.collectionId ? (
                <Link href={`/koleksiyon/${u.collectionId}`}
                  ...
                  {t('evren.viewCollection')}
                </Link>
              ) : u.directorId ? (
                <Link href={`/kisiler?q=${u.searchQuery}`}
                  className="flex-1 py-2 rounded-xl text-xs font-semibold text-center transition-all hover:opacity-80"
                  style={{ background: u.color, color: '#000' }}>
                  {t('evren.directorPage')}
                </Link>
              ) : null}
